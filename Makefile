...
run-satp-case-4:
	@echo "Running SATP Case 4: Gateway as Middleware for READ_AND_WRITE in EVM-based blockchains..."
	$(MAKE) clean-port-container PORT=3010
	# Obtain Besu binary
	#@wget -c https://github.com/hyperledger/besu/releases/download/25.11.0/besu-25.11.0.tar.gz
	#@tar -xvf besu-25.11.0.tar.gz
	#@mkdir besu-25.11.0/tmpDatadir
	# Start Besu Blockchain (port 8545 and 8546)
	(cd gateway/satp/case_4 && chmod +x startBesu.sh && ./startBesu.sh > besuout.txt &)
	sleep $(MEDIUMWAIT)
	# Start Hardhat EVM Blockchain (port 8547)
	(cd EVM && npx hardhat node --hostname 0.0.0.0 --port 8547 &)
	sleep $(MEDIUMWAIT)
	# Start the Gateway (Docker Compose)
	...
	sleep $(SHORTWAIT)
	# Deploy the SATPFungibleTokenContract to all blockchains
	(cd EVM && node scripts/SATPTokenContractCase4.js 1)
	sleep $(LONGWAIT)
	# Run the SATP protocol script (transactions, status, audit)
	...
	sleep $(MEDIUMWAIT)
	# Check (again) the balances of the user and the bridge contract address
	(cd EVM && node scripts/SATPTokenContract-CheckBalances-Case4.js)
	sleep $(SHORTWAIT)
	# Update SATPFungibleTokenContract permissions in blockchain2
	(cd EVM && node scripts/SATPTokenContractCase4.js 2)
	sleep $(LONGWAIT)
	# Run the SATP protocol script (transactions, status, audit)
